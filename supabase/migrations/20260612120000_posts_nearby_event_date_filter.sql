-- S1.3a — event-date range filter for posts_nearby.
--
-- The Sprint 1 home screen filters by date ("filtros: tipo, especie, radio,
-- fecha"). Two new trailing parameters, both defaulting to NULL so the
-- signature stays backward compatible for existing callers:
--   p_event_from / p_event_to: inclusive bounds on posts.event_date.
--
-- posts.event_date is NOT NULL (default now()::date) in the current schema,
-- so every row has a date. Should the column ever become nullable, the
-- conjuncts below already encode the intended semantics: a date filter means
-- "the event happened within this range", so NULL event_date rows would be
-- EXCLUDED while either bound is active (SQL three-valued logic: NULL >= x
-- is NULL, which fails the conjunct) and included when both bounds are NULL.
--
...
-- visibility. The event-date filters (p_event_from, p_event_to) are pure
-- AND-conjuncts on non-sensitive data — they only narrow results, never
-- widen visibility.
...
as $$
  ...
  where extensions.st_dwithin(
    ...
    -- Event-date range (inclusive). event_date is NOT NULL today; if it ever
    -- becomes nullable, NULL rows are excluded while a bound is active
    -- ("happened within range" can't be asserted for an unknown date).
